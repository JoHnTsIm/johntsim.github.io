...
let todoTextArray: string[] = [];
let todoLiDoneStateList: string[] = [];
let todoLiDoneButtonStateList: string[] = [];
...
function updateTodoList() {
    todoTextArray = []
    if (toDoList != null) {
        let liNodes = toDoList.querySelectorAll('li');

        if (liNodes.length > 0) {
            for (let index = 0; liNodes[index]; index++) {
                let node = liNodes[index];
                let textContentOfNode = node.textContent;
...
function updateTodoState() {
    ...
    if (toDoList != null) {
        ...
        if (liNodes.length > 0) {
            for (let index = 0; liNodes[index]; index++) {
                let liClass = liNodes[index].getAttribute('class');
                let liDoneButtonClass = liNodes[index].querySelectorAll('button')[1].getAttribute('class');
